...
const zoom_min = 0.1
const zoom_max = 5.0
const page_format_mm = ref<[width: number, height: number]>([210, 297])
const page_margins = ref<string>('10mm 15mm')
const display = ref('vertical')

const editor = ref<InstanceType<typeof DocumentEditor>>()
const mounted = ref(false)

const content_history = ref<string[][]>([])
...
const current_format_name = computed(() => {
    const f = formats.find(
        (f) =>
            f[1] === page_format_mm.value[0] && f[2] === page_format_mm.value[1]
    )
    return f
        ? f[0]
...
const current_text_style = computed(() =>
    // @ts-ignore
    mounted.value ? editor.value?.current_text_style : null
)

const can_undo = computed(() => undo_count.value > 0)
const can_redo = computed(
    () => content_history.value.length - undo_count.value - 1 > 0
)
...
const menu = computed(() => [
    ...
    { is: 'spacer' },
    {
        ...
        menu: formats.map(([label, w, h]) => ({
            text: label,
            active:
                page_format_mm.value[0] === w && page_format_mm.value[1] === h,
            click: () => (page_format_mm.value = [w as number, h as number]),
        })),
    },
    {
...
watch(
    ...
    (newVal) => {
        if (!_mute_next_content_watcher) {
            content_history.value[++undo_count.value] = structuredClone(
                toRaw(newVal)
            )
...
onMounted(() => {
    ...
    window.addEventListener('gesturechange', (e) => {
        e.preventDefault()
        zoom.value = Math.min(
            Math.max(gestureStartZoom * e.scale, zoom_min),
            zoom_max
        )
    })
    ...
    window.addEventListener(
        ...
        (e) => {
            if (e.touches.length === 2 && startDist) {
                ...
                zoom.value = Math.min(
                    Math.max((startZoom * newDist) / startDist, zoom_min),
                    zoom_max
                )
            }
        },
